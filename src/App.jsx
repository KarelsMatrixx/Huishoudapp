...
function staatOpen(klus: any, vandaag: string) {
  ...
  if (klus.ritme === "week" || klus.ritme === "weekvrij") return maandagVan(klus.laatst) !== maandagVan(vandaag);
...
function vervaldag(klus: any, t: ReturnType<typeof nuNL>): string | null {
  ...
  if (klus.ritme === "weekvrij") return null; // hele week open, geen herinnering per dag
...
Deno.serve(async () => {
  ...
  for (const klus of klussen) {
    ...
    const samenwerk = klus.wie === "samen" || klus.wie === "beiden";
    const doel = samenwerk ? abos : abos.filter((a) => a.persoon === klus.wie);
    const n = await stuur(doel.length ? doel : abos, {
      titel: soort === "morgen" ? `Morgen: ${klus.titel}` : klus.titel,
      tekst: soort === "morgen"
        ? `${klus.wie === "beiden" ? "Gezamenlijk, allebei afvinken" : klus.wie === "samen" ? "Samen" : naam ?? "Jij"} · ${klus.punten ?? 1} punten`
